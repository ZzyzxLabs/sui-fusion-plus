...
export class STABLECOIN implements StructClass {
  ...
  static reified(): STABLECOINReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        STABLECOIN.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        STABLECOIN.fromFieldsWithTypes(item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        STABLECOIN.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        STABLECOIN.fromSuiObjectData(content),
      fetch: async (client: SuiClient, id: string) =>
        STABLECOIN.fetch(client, id),
  ...
  static fromSuiParsedData(content: SuiParsedData): STABLECOIN {
    ...
    if (!isSTABLECOIN(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a STABLECOIN object`
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<STABLECOIN> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching STABLECOIN object at id ${id}: ${res.error.code}`
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isSTABLECOIN(res.data.bcs.type)
    ) {
