...
export class USDC implements StructClass {
  ...
  static reified(): USDCReified {
    return {
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        USDC.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        USDC.fromSuiObjectData(content),
  ...
  static fromSuiParsedData(content: SuiParsedData): USDC {
    ...
    if (!isUSDC(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a USDC object`
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<USDC> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching USDC object at id ${id}: ${res.error.code}`
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isUSDC(res.data.bcs.type)
    ) {
