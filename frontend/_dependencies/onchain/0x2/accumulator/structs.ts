...
export function isAccumulatorRoot(type: string): boolean {
  ...
  return type === `${PKG_V34}::accumulator::AccumulatorRoot`;
}
...
export class AccumulatorRoot implements StructClass {
  ...
  static readonly $typeName = `${PKG_V34}::accumulator::AccumulatorRoot`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = AccumulatorRoot.$typeName;
  readonly $fullTypeName: `${typeof PKG_V34}::accumulator::AccumulatorRoot`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: AccumulatorRootFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs
    ) as `${typeof PKG_V34}::accumulator::AccumulatorRoot`;
  ...
  static fromSuiParsedData(content: SuiParsedData): AccumulatorRoot {
    ...
    if (!isAccumulatorRoot(content.type)) {
      throw new Error(
        ...
        } is not a AccumulatorRoot object`
      );
    }
...
export function isKey(type: string): boolean {
  type = compressSuiType(type);
  return type.startsWith(`${PKG_V34}::accumulator::Key` + "<");
}
...
export type KeyReified<T0 extends PhantomTypeArgument> = Reified<
  Key<T0>,
  KeyFields<T0>
>;
...
export class Key<T0 extends PhantomTypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V34}::accumulator::Key`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [true] as const;

  readonly $typeName = Key.$typeName;
  readonly $fullTypeName: `${typeof PKG_V34}::accumulator::Key<${PhantomToTypeStr<T0>}>`;
  ...
  private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: KeyFields<T0>) {
    this.$fullTypeName = composeSuiType(
      Key.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V34}::accumulator::Key<${PhantomToTypeStr<T0>}>`;
    this.$typeArgs = typeArgs;
  ...
  static reified<T0 extends PhantomReified<PhantomTypeArgument>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        Key.$typeName,
        ...[extractType(T0)]
      ) as `${typeof PKG_V34}::accumulator::Key<${PhantomToTypeStr<
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Key.fromFieldsWithTypes(T0, item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Key.fromSuiParsedData(T0, content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Key.fromSuiObjectData(T0, content),
...
export function isU128(type: string): boolean {
  type = compressSuiType(type);
  return type === `${PKG_V34}::accumulator::U128`;
}
...
export class U128 implements StructClass {
  ...
  static readonly $typeName = `${PKG_V34}::accumulator::U128`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = U128.$typeName;
  readonly $fullTypeName: `${typeof PKG_V34}::accumulator::U128`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: U128Fields) {
    this.$fullTypeName = composeSuiType(
      U128.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V34}::accumulator::U128`;
    this.$typeArgs = typeArgs;
  ...
  static reified(): U128Reified {
    return {
      ...
      fromBcs: (data: Uint8Array) => U128.fromBcs(data),
      bcs: U128.bcs,
  ...
  static async fetch(client: SuiClient, id: string): Promise<U128> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching U128 object at id ${id}: ${res.error.code}`
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isU128(res.data.bcs.type)
    ) {
