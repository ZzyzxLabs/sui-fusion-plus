...
import {
  TRUE_ERC20,
  ESCROW_FACTORY,
} from "@/cross-chain-sdk-custom/cross-chain-sdk/src/deployments";
  ...
  if (chain === "ETH") {
    ...
    return (
      <svg className='w-6 h-6' viewBox='0 0 32 32'>
        ...
        <path
          fill='#FFF'
          d='M16 7l-4.5 2.598v10.804L16 23l4.5-2.598V9.598L16 7zm3.5 11.902L16 21.5l-3.5-2.598v-8.608L16 8.5l3.5 1.794v8.608z'
        />
      </svg>
    );
  }
...
export default function Home() {
  ...
  const findMatchingCoins = () => {
    ...
    console.log(
      `Found ${matchingCoins.length} matching coins`,
      matchingCoins,
      targetCoinType
    );
    ...
    try {
      const response = await fetch(
        `https://api.coingecko.com/api/v3/simple/price?ids=${getCoinId(
          srcChain
        )},${getCoinId(dstChain)}&vs_currencies=usd`
      );
  ...
  const createEVMOrder = async () => {
    ...
    if (srcChain !== "ETH" || dstChain !== "SUI") {
      setError(
        "This function only supports ETH as source and SUI as destination chain."
      );
      return;
    }
      ...
      const order = CrossChainOrder.new(
        ...
        },
        {
          ...
          whitelist: [
            {
              address: new Address(
                "0x0000000000000000000000000001100000000000"
              ), // Placeholder for whitelisted
              allowFrom: 0n,
            },
          ],
