...
class ResolverService {
  ...
  private async processOrder(order: Order): Promise<void> {
    ...
    try {
      ...
      // Step 3: Execute fill order/interaction
      await this.executeFillOrder(order, auctionInfo, escrows);
      logger.info('Fill order executed successfully', null, { orderId });

      // Step 4: Wait for transaction confirmations
      await this.waitForConfirmations(escrows);
      logger.info('Transaction confirmations received', null, { orderId });

      // Step 5: Verify bilateral escrow status
      const verificationResult = await this.verifyEscrowStatus(order, escrows);
      logger.info('Escrow verification completed', verificationResult, { orderId });

      if (verificationResult.verified) {
        // Step 6: Notify backend to verify
        ...
        logger.info('Order completed successfully', null, { orderId });
      } else {
        logger.error('Escrow verification failed', verificationResult.issues, { orderId });
  ...
  /**
   * Execute fill order and LOP interaction
   */
  private async executeFillOrder(
    order: Order, 
    auctionInfo: AuctionInfo, 
    escrows: { source: EscrowDeployment; destination: EscrowDeployment }
  ): Promise<void> {
    const orderId = order.id;
    logger.info('Executing fill order', null, { orderId, chain: auctionInfo.sourceChain });

    try {
      // Call LOP fillOrder function based on source chain
      if (auctionInfo.sourceChain === 'sui') {
        await this.fillOrderOnSui(order, escrows.source);
      } else {
        await this.fillOrderOnEvm(order, escrows.source);
      }

      logger.info('Fill order execution completed', null, { orderId, chain: auctionInfo.sourceChain });
    } catch (error) {
      logger.error('Failed to execute fill order', error, { orderId, chain: auctionInfo.sourceChain });
      throw error;
    }
  }

  /**
   * Fill order on Sui chain
   */
  private async fillOrderOnSui(order: Order, escrow: EscrowDeployment): Promise<void> {
    const orderId = order.id;
    
    try {
      const result = await this.chainInteractions.sui.fillOrder({
        order: order.order,
      ...
      logger.info('Sui fill order completed', result, { 
        orderId, 
        chain: 'sui', 
        txHash: result.txHash 
      });
    } catch (error) {
      logger.error('Failed to fill order on Sui', error, { orderId, chain: 'sui' });
      throw error;
    }
  }

  /**
   * Fill order on EVM chain
   */
  private async fillOrderOnEvm(order: Order, escrow: EscrowDeployment): Promise<void> {
    const orderId = order.id;
    
    try {
      const result = await this.chainInteractions.evm.fillOrder({
      ...
      logger.info('EVM fill order completed', result, { 
        orderId, 
        chain: 'evm', 
        txHash: result.txHash 
      });
    } catch (error) {
      logger.error('Failed to fill order on EVM', error, { orderId, chain: 'evm' });
      throw error;
    }
  }

  /**
   * Wait for transaction confirmations on both chains
   */
  private async waitForConfirmations(escrows: {
    source: EscrowDeployment;
    destination: EscrowDeployment;
  }): Promise<void> {
    logger.info('Waiting for transaction confirmations');

    const promises = [];

    // Wait for source chain confirmations
    if (escrows.source.chain === 'sui') {
      promises.push(this.chainInteractions.sui.waitForConfirmation(
        escrows.source.transactionHash, 
        this.config.maxRetries
      ));
    } else {
      promises.push(this.chainInteractions.evm.waitForConfirmation(
        escrows.source.transactionHash, 
        this.config.confirmationBlocks.evm
      ));
    }

    // Wait for destination chain confirmations
    if (escrows.destination.chain === 'sui') {
      promises.push(this.chainInteractions.sui.waitForConfirmation(
        escrows.destination.transactionHash, 
        this.config.maxRetries
      ));
    } else {
      promises.push(this.chainInteractions.evm.waitForConfirmation(
    ...
    if (results.every(result => result)) {
      logger.info('All transaction confirmations received');
    } else {
      throw new Error('One or more transactions failed confirmation');
    }
  }
  ...
  /**
   * Verify bilateral escrow status
   */
  private async verifyEscrowStatus(
    order: Order,
    escrows: { source: EscrowDeployment; destination: EscrowDeployment }
    ...
    try {
      ...
      const sourceValid = await this.verifyIndividualEscrow(escrows.source, 'source');
      if (!sourceValid.isValid) {
        issues.push(...sourceValid.issues);
      }

      // Verify destination escrow
      const destValid = await this.verifyIndividualEscrow(escrows.destination, 'destination');
      if (!destValid.isValid) {
        issues.push(...destValid.issues);
      }

      // Check balance consistency
      const balanceConsistent = await this.verifyBalanceConsistency(escrows);
      if (!balanceConsistent.isValid) {
        issues.push(...balanceConsistent.issues);
      }
  ...
  /**
   * Verify individual escrow status
   */
  private async verifyIndividualEscrow(
    ...
    try {
      if (escrow.chain === 'sui') {
        // Verify Sui escrow using chain interaction utility
        const status = await this.chainInteractions.sui.getEscrowStatus(escrow.escrowAddress);
        
        if (!status.exists) {
          issues.push(`Sui escrow ${escrow.escrowAddress} not found`);
        } else {
          logger.debug(`Sui ${type} escrow status`, status);
          // Add additional validation logic based on status
          if (!status.locked && type === 'source') {
            issues.push(`Source escrow ${escrow.escrowAddress} should be locked`);
          }
        }
      } else {
        // Verify EVM escrow using chain interaction utility
        const status = await this.chainInteractions.evm.getEscrowStatus(escrow.escrowAddress);
        
        if (!status.exists) {
          issues.push(`EVM escrow ${escrow.escrowAddress} is not a contract`);
        } else {
          logger.debug(`EVM ${type} escrow status`, status);
          // Add additional validation logic based on status
          if (!status.locked && type === 'source') {
            issues.push(`Source escrow ${escrow.escrowAddress} should be locked`);
          }
          if (status.balance === '0') {
            issues.push(`Escrow ${escrow.escrowAddress} has zero balance`);
          }
        }
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      issues.push(`Failed to verify ${type} escrow: ${message}`);
      logger.error(`Escrow verification error`, error);
    }
  ...
  /**
   * Verify balance consistency between escrows
   */
  private async verifyBalanceConsistency(escrows: {
    source: EscrowDeployment;
    destination: EscrowDeployment;
  }): Promise<{ isValid: boolean; issues: string[] }> {
    // This would implement cross-chain balance verification logic
